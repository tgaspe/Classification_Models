...
```{r unconstrained_linear_model, message=FALSE, warning=FALSE, include=FALSE}
...
```{r lasso_model, message=FALSE, warning=FALSE, include=FALSE}
...
```{r ridge_model, message=FALSE, warning=FALSE, include=FALSE}
...
At the same time, with the threshold of 0.8, the GAM model performed better overall. It achieved an accuracy of 89%, a sensitivity of 85%, and a specificity of 90%. This suggests that the GAM model correctly captured the non linear effects in the data and performed overall better than the lasso and ridge models.
...
```{r step_wise_predictors_selection, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
library(leaps)
...
```{r step_wise_best_predictors, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
library(car)
...
```{r gam_step_wise, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# GAM model 
library(gam)
gam_model.stepwise <- gam(LET_IS ~ s(AGE) + s(STENOK_AN) + s(GB) + ZSN_A + s(S_AD_ORIT) + K_SH_POST + s(ant_im) + s(inf_im) + IM_PG_P + s(n_p_ecg_p_12) + s(L_BLOOD) + s(TIME_B_S) + s(NA_KB) + NITR_S + ASP_S_n, data = MI_train, family = binomial)

summary(gam_model.stepwise)
plot(gam_model, se = TRUE, col = "purple")

# Predictions
gam_pred.stepwise <- predict(gam_model, MI_test, type = "response")

gam_pred.stepwise <- ifelse(gam_pred > 0.5, TRUE, FALSE)

# Confusion matrix
gam_model.table <- table(MI_test$LET_IS, gam_pred.stepwise)
confusionMatrix(rotate_table_180(gam_model.table))
```

```{r neural_network, eval=FALSE, include=FALSE}
# Neural Network
library(neuralnet)

# Neural Network model
#nn_model <- neuralnet(LET_IS ~ ., data = MI_train, hidden = c(5, 3), act.fct = "logistic", linear.output = FALSE)
#plot(nn_model)
